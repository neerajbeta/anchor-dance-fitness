...
export function EnquiriesClient() {
  ...
  return (
    <div className="card">
      <div className="card-title">📨 Book a Demo — Enquiries</div>

      {items.length === 0 ? (
        <div className="rounded-lg border-[1.5px] border-dashed border-line bg-cream/40 py-10 text-center text-[13px] text-muted">
          No enquiries yet — submissions from the &quot;Book a Demo&quot; form will appear here.
        </div>
      ) : (
        <div className="flex flex-col gap-2.5">
          {items.map((e) => (
            <div key={e.id} className="rounded-lg border-[1.5px] border-line bg-white p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-[12px] text-muted">
                    {e.email} · {e.phoneCountryCode ?? ""} {e.phone}
                  </div>
